...
pub fn build_minimal_stack(program_path: &str) -> *const u8 {
    let stack = rt_mmap_rw(STACK_SIZE);
    let base = stack as usize;
    let top = base
        .checked_add(STACK_SIZE)
        .unwrap_or_else(|| fatal("stack top overflow"));
    ...
    let path_total = path_bytes
        .len()
        .checked_add(1)
        .unwrap_or_else(|| fatal("program path length overflow"));
    ...
    cursor = cursor
        ...
        .unwrap_or_else(|| fatal("stack underflow while writing argv[0]"));
...
fn push_u64(cursor: &mut usize, stack_base: usize, value: u64) {
    *cursor = cursor
        .checked_sub(8)
        .unwrap_or_else(|| fatal("startup stack underflow"));

    if *cursor < stack_base {
        fatal("startup stack write before base");
    }
