VERUS_BIN := .verus/verus-x86-linux/verus
VERUS_ENV := MAKEFLAGS=

VERILOAD_MAIN := src/s0_main_impl.rs
VERILOAD_SOURCES := $(wildcard src/*.rs)
VERILOAD_BINARY := $(BUILD_DIR)/veriload

$(VERILOAD_BINARY): $(VERILOAD_SOURCES) | $(BUILD_DIR)
	$(VERUS_ENV) $(VERUS_BIN) --compile $(VERILOAD_MAIN) -- -C target-feature=+crt-static -o $(VERILOAD_BINARY)

.PHONY: verify
verify:
	$(VERUS_ENV) $(VERUS_BIN) $(VERILOAD_MAIN)

.PHONY: compile
compile: $(VERILOAD_BINARY)
